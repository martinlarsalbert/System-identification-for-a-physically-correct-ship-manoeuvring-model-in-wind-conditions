...
The PI model was identified with inverse dynamics (ID) regression. Two other models where also identified for comparisons;
The reference model has the same model structure as PI model, but with parameters identified with VCT regression; The PU model served as a reference for an uninformed model identified in the same way as PISM.
...
As a logical consequence of this, the PU model is considered as physically incorrect, since it predicted very different forces and moments; The regression seems to have made an incorrect decomposition between hull-, and rudder forces, as well as drift angle-, and yaw rate dependent coefficients.
The PI model on the other hand, predicted very similar forces compared to the reference model for the model tests and can therefore considered as a more physically correct model.
