...
%The regression pipelines of the mathematical model (\autoref{tab:vct_pipeline_abkowitz}), and the PISM (\autoref{tab:vct_pipeline_semiempirical}) are different. 
The regression pipeline of the PISM is shown in \autoref{tab:vct_pipeline_semiempirical}. The semi-empirical rudder is treated as a deterministic model and is therefore not included in the regression. The regression is instead performed on the hull forces $X_H,Y_H,N_H$ obtained from the VCT calculations. The total sway force $Y_D$, and total yawing moment $N_D$ are however used to determine the rudder hull interaction coefficients ${a_H}',{x_H}'$. 
\begin{table}[h!]
    \centering
...
%\begin{table}[h!]
%    \centering
%    \caption{Pipeline for the VCT regression of the mathematical model}
...
\begin{table}[h!]
    \centering
    \caption{Pipeline for the VCT regression of the PISM}
